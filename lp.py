...
csv_name = 'data/datasets/lp-' + version_name + '.csv'
if os.path.exists(csv_name): os.remove(csv_name)

model_name = 'data/models/' + version_name + '.pkl'
model = pickle.load(open(model_name, 'rb')); row = [np.NaN] * 55

def send_packs():
    global running, pending_acks, send_lock, df, model, row
    global curr_packet, last_packet, sent_packets, lost_packets
    global MSS, cwnd, ssthresh, cwnd_order, last_ack, ewma_smoothing_factor, lpthresh
